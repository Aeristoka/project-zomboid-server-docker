        ...
        if [ -d "$search_dir" ]; then
            ...
            if [ -d "$item/mods" ]; then
                for mod_folder in "$item/mods"/*; do
                    for internal_folder in "$mod_folder"/*; do
                        if [ -d "$internal_folder/media/maps" ]; then

                            # Copy maps to map folder
                            source_dirs=("$internal_folder/media/maps"/*)
                            ...
                            for source_dir in "${source_dirs[@]}"; do
                                ...
                                if [ ! -d "$map_dir/$dir_name" ]; then
                                    ...
                                    cp -r "$internal_folder/media/maps"/* "${HOMEDIR}/pz-dedicated/media/maps"
